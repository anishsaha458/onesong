...
// All functions called via onclick in index.html are plain
// globals. No ES module import/export — avoids scope issues.
// ============================================================
...
// Expose toast helpers so ambient.js can show analysis progress
...
function showSongSelection() {
    ...
    ['input-song-name', 'input-artist-name', 'input-youtube-url']
...
async function getRecommendations() {
    ...
    try {
        ...
        if (!r.ok) { recError.textContent = data.detail || 'Could not fetch recommendations.'; recError.classList.remove('hidden'); return; }
        if (!data.tracks || !data.tracks.length) { recError.textContent = 'No similar songs found.'; recError.classList.remove('hidden'); return; }
